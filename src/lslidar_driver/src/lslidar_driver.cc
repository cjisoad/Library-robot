...
namespace lslidar_driver
{
	...
	{
		stop_pubscan_thread_.store(true);
	...
	{
		stop_pubscan_thread_.store(false);
	...
	void LslidarDriver::pubScanThread()
	{
		...
		while (rclcpp::ok() && !stop_pubscan_thread_.load())
		{

			while (wait_for_wake && !stop_pubscan_thread_.load())
			{
				pubscan_cond_.wait(lock);
				wait_for_wake = false;
			}
			if (stop_pubscan_thread_.load())
			{
				break;
			}
				if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
				{
					if (pubScan)
					{
						...
						int scan_num = lidar_name == "N10_P" ? count_num : count_num * 2;
						if (scan_num < 2 || count_num <= 0)
						{
							wait_for_wake = true;
							continue;
						}
						...
						if (points.size() < 3000 + static_cast<size_t>(count_num))
						{
							wait_for_wake = true;
							continue;
						}
					...
					scan->time_increment = scan_time / (double)(scan_num - 1);
					...
					for (int k = 0; k < scan_num; k++)
					{
						...
						for (int i = 0; i < count_num; i++)
						{
							...
							if (lidar_name == "N10_P")
							{
								bool first_valid = points[i].range > 0.0;
								bool second_valid = points[i + 3000].range > 0.0;
							if (!first_valid && !second_valid)
							{
								scan->ranges[point_idx] = std::numeric_limits<float>::infinity();
								scan->intensities[point_idx] = 0;
							}
							else if (!second_valid || (first_valid && points[i].range <= points[i + 3000].range))
							{
							...
							{
								scan->ranges[point_idx] = (float)points[i + 3000].range;
								scan->intensities[point_idx] = points[i + 3000].intensity;
							}
						}
							else
							{
								int second_idx = point_idx + count_num;
								if (second_idx >= scan_num)
									continue;
							...
							else
							{
								...
								{
									scan->ranges[second_idx] = std::numeric_limits<float>::infinity();
									scan->intensities[second_idx] = 0;
								}
								else
								{
									double dist = points[i + 3000].range;
									scan->ranges[second_idx] = (float)dist;
									scan->intensities[second_idx] = points[i + 3000].intensity;
								}
							}
						}
