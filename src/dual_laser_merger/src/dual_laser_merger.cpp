...
bool MergerNode::transform_cloud_to_target(
  ...
  double yaw_offset)
{
  if (cloud.header.frame_id == target_frame_param) {
    return true;
  }

  if (enable_calibration_param) {
    tf2_msg.header = cloud.header;
    tf2_msg.child_frame_id = cloud.header.frame_id + "_calibrated";
    tf2_msg.transform.translation.x = x_offset;
    tf2_msg.transform.translation.y = y_offset;
    tf2_msg.transform.translation.z = 0.0;
    tf2_quaternion.setRPY(0, 0, yaw_offset);
    ...
    cloud.header.frame_id = tf2_msg.child_frame_id;
  }

  try {
    cloud = tf2_buffer->transform(
      cloud, target_frame_param, tf2::durationFromSec(tolerance_param));
  } catch (tf2::TransformException & ex) {
    RCLCPP_ERROR_STREAM(
      this->get_logger(), "Transform failure, " << laser_name << ": " << ex.what());
    return false;
  }
  ...
  const sensor_msgs::msg::LaserScan::ConstSharedPtr & lidar_2_msg)
{
  ...
  if (target_frame_param.empty()) {
    ...
    if (!transform_cloud_to_target(
        cloud_in_1, "Laser 1", laser_1_x_offset, laser_1_y_offset, laser_1_yaw_offset))
    {
      return;
    }

    if (!transform_cloud_to_target(
        cloud_in_2, "Laser 2", laser_2_x_offset, laser_2_y_offset, laser_2_yaw_offset))
    {
      return;
    }
